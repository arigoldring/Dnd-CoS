...
import { useMaps, CampaignMap } from "../../hooks/useMaps";
...
/**
 * One map's viewer: the image, its pins, and the DM's editing/reveal tools for
 * them. The index at Maps.tsx is what decides WHICH maps this viewer may even
 * be reached for; this component only has to resolve one, given a :mapId that
 * already survived that gate once.
 */
export function MapView() {
  const { loading: authLoading } = useAuth();
  // Non-null by construction — this route sits under CampaignLayout. Scopes
  // the pin fetch to this campaign so a DM in two campaigns doesn't get both
  // campaigns' pins merged onto one map.
  const campaign = useCampaign();
  const { previewing } = usePlayerPreview();
  const { mapId } = useParams();

  // Not `previewing`, and this page is the one place that distinction matters.
  // On every other reveal-gated read RLS already dropped what a player may not
  // see, so asPlayer only reproduces that for a DM. The map registry is
  // client-side — there is no RLS to have done it — so this flag is the ONLY
  // thing hiding a map from a real player. See src/data/maps.ts.
  ...
  // Still plain `previewing` here: pin reveal IS RLS-gated, so a real player's
  // response never carried the hidden rows.
  const {
    data: locations = [],
  ...
  // mapsLoading is not optional here. Without it the first render has
  // maps = [], the find below misses, and every hard refresh of a viewer URL
  // flashes to the index before the real answer arrives.
  if (authLoading || mapsLoading || locationsLoading) return <p>Loading...</p>;
  const error = mapsError?.message ?? locationsError?.message;
  if (error) return <p>{error}</p>;

  // One lookup answers both "is there such a map" and "may this viewer open
  // it": an unknown :mapId misses, a hidden one was already filtered out of
  // `maps` above. `replace` so Back doesn't bounce you off the index again.
  const map = maps.find((m) => m.id === mapId);
  if (!map) return <Navigate to={`/campaign/${campaign.id}/Maps`} replace />;

  const pins = locations.filter((loc) => loc.mapKey === map.id);

  return (
    // key: same route matches every map, so React would otherwise reuse this
    // instance across a :mapId change and carry selectedId, peek, editing and
    // the zoom/pan transform from one map onto the next. Not reachable from
    // the index today (it unmounts on the way through), but it is the
    // codebase's own idiom for exactly this hazard — see key={campaign.id} on
...
function MapViewInner({
  ...
  // pins, not locations — a stale selectedId left over from another map would
  // otherwise open a drawer for a pin that isn't on this one.
  const selected = pins.find((loc) => loc.id === selectedId) ?? null;
  ...
  // previewing DM's, so `pins` is already exactly what should be drawn — that
  // half of this page is exactly as dumb as it always was. The map image
  // itself is the one thing on this page that is NOT covered by that sentence;
  ...
  return (
    <div className="maps has-peek">
      <div className="maps-header">
        <Link className="maps-back" to={`/campaign/${campaign.id}/Maps`}>
          ← Maps
        </Link>
        <p className="maps-title">{map.title}</p>
        {showDmUi && (
          <button
        ...
        >
          ...
          >
            <div
              ...
              // DM-only coordinate finder: click (or drag-start) anywhere on the
              // map and its x%/y% is logged to the console, ready to paste into
              // an `insert into locations (...)` for this map. Percentages, so
              // the numbers are scale-invariant — offset and rect grow together
              // with zoom. Capture phase, so this fires before a hotspot's
              // onClick and before panning starts — which means a pan-drag also
              // logs a coordinate; harmless, and not worth suppressing.
              onMouseDownCapture={
                showDmUi
                  ? (e) => {
                      ...
                      console.log(`MAP COORDS [${map.id}]:`, x.toFixed(1), y.toFixed(1));
                    }
                  : undefined
              }
            >
...
// in MapViewInner so that unmounting it is the whole of "cancel" — nothing to
// reset — and a keystroke in the textarea re-renders the form, not the map.
